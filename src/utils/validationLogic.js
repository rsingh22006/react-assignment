...
export const handleCheckName = (name) => {
    ...
    if (name?.length > 0 && !ALPHA_REGEX.test(name)) {
        return 'NAME is invalid, only alphabates are allowed!';
    }
    if (name?.length > 0 && name?.length < 3) {
        return 'NAME is invalid, it should contain minimum 3 characters!';
    }
}
...
export const handleCheckEmail = (email) => {
    ...
    if (email?.length > 0) {
        ...
        if (!isFirstCharRight) {
            return 'Your EMAIL first word should be a-z or 0-9';
        }
...
export const handleCheckPhoneNumber = (phoneNumber) => {
    ...
    if (phoneNumber?.length > 0) {
        if (!ALPA_NUM.test(phoneNumber)) {
            return 'PHONE NO. is invalid, remove "." from it';
        }
        if (phoneNumber.length > 10 || phoneNumber.length < 10) {
            return 'Your PHONE NO. is invalid, it must include 10 numbers';
        }
    }
}
