...
export const InputField = ({ name, type = 'text', value, labelValue, show, error, handleChange, handleClickShow }) => {
    ...
    return (
        ...
        <label
            htmlFor={name}
            className={`absolute select-none text-gray-500 duration-300 transform -translate-y-4 scale-75 top-4 origin-[0] left-[3.5%] peer-focus:text-sm ${error ? 'peer-focus:text-error-color text-error-color' : 'peer-focus:text-[#0a856d]'} peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-4`}
        >{labelValue}
        </label>
